...
- 2026-05-21 — **`pdm add -dG dev -e "file:///${PWD}/..."` writes a host-absolute path into `pyproject.toml`.** Phase 7 ran `pdm add -dG dev -e "file:///${PWD}/apps/test-data-agent/python"`. Shell expanded `$PWD` BEFORE pdm saw it, so `/Users/nbaynham/projects/...` got baked into `[tool.pdm.dev-dependencies]`. Local installs worked; CI runners (`/home/runner/...`) failed `pdm install` for **every push from Phase 7 through Phase 10**. The README badge was lying — it really fetches the live SVG, which had been red the whole time. We only caught it when we explicitly checked the badge contents via `curl`. Resolution: replace the absolute path with `file:///${PROJECT_ROOT}/...`. PDM expands `${PROJECT_ROOT}` itself at install time. **General rules**: (1) never let `$PWD` reach a config file — use literal `${PROJECT_ROOT}` (PDM substitutes it) or a path-style `./relative/path` if the tool supports it; (2) "the README badge is green" is not the same as "CI is green" — verify the actual badge SVG, especially after pyproject.toml edits.
- 2026-05-21 — **`pdm.lock` content_hash gets stale unless `pdm lock --refresh` is run after every pyproject.toml dep edit.** Even after the URL fix in `016a279`, CI still failed because PDM noticed the lockfile hash didn't match the (now-corrected) pyproject.toml and tried to re-resolve from the URL on the CI runner — where the same URL parsing quirk hit again. The lockfile itself stored a portable relative path (`./apps/test-data-agent/python`) — CI just never got to USE it because the hash check failed first. Resolution: enforce the freshness invariant in two places. **(a) Pre-commit hook** runs `pdm lock --check` so a commit with a stale lockfile fails locally before push. **(b) `make ci-status` target** queries the actual GitHub check-runs API (not the cached badge SVG) and prints per-job status, so any future "is CI green?" question gets a real answer in under a second. **General rule**: lockfiles are content-addressed. Edits to pyproject.toml without `pdm lock --refresh` produce silently-broken commits that can pass `pdm install` locally (it uses the existing lockfile) but fail CI (which re-resolves from pyproject.toml because the hash drift looks like an out-of-date lock).
